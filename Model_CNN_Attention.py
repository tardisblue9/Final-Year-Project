...
class Net(nn.Module):
    def __init__(self):
        ...
        # self.conv_art1 = nn.Conv2d(in_channels = 1, out_channels = 450, kernel_size = (2, 450), stride = (1 , 1)) 

        self.art_dim = 150*3
        self.fc_art_1 = nn.Linear(self.art_dim, 192)
...
#         print("sentence number:",len(x))
        ...
        for x_sent in x:
            try:
                # pass1
                temp_x_sent1 = self.conv1(x_sent) # convolution
                temp_x_sent1,_ = torch.max(temp_x_sent1,dim = 2,keepdim=True) # pooling
                # pass2
                temp_x_sent2 = self.conv2(x_sent)
                temp_x_sent2,_ = torch.max(temp_x_sent2,dim = 2,keepdim=True)
                # pass3
                temp_x_sent3 = self.conv3(x_sent)
                temp_x_sent3,_ = torch.max(temp_x_sent3,dim = 2,keepdim=True)
                # conbine
                temp_x_sent_comb = torch.cat([temp_x_sent1,temp_x_sent2,temp_x_sent3], 1)
                temp_x.append(temp_x_sent_comb)
            except:continue # in case sentence is too short (length<2),pass this sentence
        x = torch.cat(temp_x, 2)
        x = torch.transpose(x,1,3)


        w_art = F.relu(self.fc_art_1(x))
        w_art = self.dp1(w_art)
        w_art = self.fc_art_2(w_art)
        F.softmax(w_art,dim=2) # normalize
        x = torch.mul(x, w_art) # element wise multiply
        x = torch.sum(x, 2, True)  # art_dim*1
...
save_fig(logbook_epoch, "/Users/vanellope/Desktop/FYP/Plot归档/embedBert_level2_CNN_attention_2_5_8_150c_450_lr{}_epoch{}.jpg".format(learning_rate,epoch_number))
